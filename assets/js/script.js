...
// Map
            ...
            // const arrow = document.getElementById('connectivityArrow');
            ...
            /*const content = document.querySelector('.connectivityContent');
            const arrow = document.querySelectorAll(".connectivityArrow");
            document.querySelectorAll(".map-connectivity-section").forEach((element,index)=>{
                element.addEventListener("click",()=>{
                    if (content[index] && content[index].classList.contains('map-active')) {
                        content[index].classList.remove('map-active');
                        arrow[index].textContent = '▶';
                    } else if (content[index]) {
                        content[index].classList.add('map-active');
                        arrow[index].textContent = '▼';
                    }
                })
            })*/
            ...
            //     document.querySelectorAll(".map-connectivity-header").forEach((element, index) => {
            //         element.addEventListener("click", () => {
            //             console.log(index);
            ...
            //             if (content.classList.contains("map-active")) {
            //                 console.log("true");
            //                 content.classList.remove("map-active");
            //                 arrow.textContent = '▶';
            //                 return;
...
document.addEventListener("DOMContentLoaded", function () {
    let modalFromOverlay = document.getElementById("modalFromOverlay"),
        ...
        modalFromTriggerButtons = document.querySelectorAll(
            ".offerBoxEnquireButton, .aboutEnquireBtn, #overviewRequestBtn, #priceBreakupBtn, #floorPlan, .floorPlan-button, .amenitiesDownloadBtn, .galleryDownloadButton, .firstFormCallBack, .floorPlan-card, .virtual1"
        );
    ...
    function modalFromOpen(event, title = "GOT QUESTIONS?") {
        ...
        let modalCurrentTitle = title,
            modalTargetElement = event ? event.target.closest("button, a, .virtual1, #priceBreakupBtn, #floorPlan") : null;
        ...
        if (modalTargetElement) {
            if (modalTargetElement.classList.contains("offerBoxEnquireButton") || modalTargetElement.classList.contains("aboutEnquireBtn")) {
                ...
                modalFromContainer.classList.add("modalDownload");
            } else if (modalTargetElement.classList.contains("virtual1")) {
